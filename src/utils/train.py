...
from utils.utils import rand_bbox, project_bin_mask
...
class Trainer_Masked_ViT(object):
    def __init__(self, model, args):
        wandb.config.update(args)
        self.device = args.device
        self.clip_grad = args.clip_grad
...
#         projected_mask = project_bin_mask(img, index, self.data, self.lambda_drop, self.ps, img.shape[1], self.hidden_size)
#         projected_mask = create_binary_mask(projected_mask).to(self.device)
...
#             # compute output
#             with torch.cuda.amp.autocast():
#                 out, _ = self.model(img, projected_mask)
#                 loss = self.criterion(out, target_a) * lam + self.criterion(out, target_b) * (1. - lam)
#         else:
#             # compute output
#             with torch.cuda.amp.autocast():
...
#         projected_mask = project_bin_mask(img, index, self.test_data, self.lambda_drop, self.ps, img.shape[1], self.hidden_size)
#         projected_mask = create_binary_mask(projected_mask).to(self.device)

#         # projected_mask = torch.ones_like(self.model.patch_emb(img)).to(self.device)
