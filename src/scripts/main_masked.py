...
from utils.train import Trainer_Masked

parser = argparse.ArgumentParser()
parser.add_argument('--dataset', required=True, choices=['c10', 'c100', 'svhn'])
parser.add_argument('--model', required=True, choices=['mlp_mixer', 'mlp_mixer_masked'])
...
experiment_name = f"{args.model}_{args.dataset}_{args.optimizer}_{args.scheduler}_224"
if args.autoaugment:
    experiment_name += "_aa"
if args.clip_grad:
...
if __name__=='__main__':
    with wandb.init(project='mlp_mixer', config=args, name=experiment_name):
        ...
        trainer = Trainer_Masked(model, args)
